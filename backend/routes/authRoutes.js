...
router.post('/pair', async (req, res) => {
    try {
        const { token, name, scannerId: existingId } = req.body;
        ...
        if (token === SETUP_SECRET) {
            ...
            // Fingerprint never expires and is tied to the device identity
            tokenScanner = await Scanner.findOne({ fingerprint: token });
            if (tokenScanner) {
                isRepairToken = true;
                // If the token IS the fingerprint, force the context to be that scanner
                if (!existingId) {
                    req.body.scannerId = tokenScanner.uuid; // Inject ID from fingerprint
                }
            } else {
